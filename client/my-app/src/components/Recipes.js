...
const Recipes = () => {
  ...
  const getRecipes = () => {
    fetch("http://localhost:5100/auth/recipe", {
      method: "GET",
      headers: {
    ...
    try {
      ...
      if (window.confirm("Are you sure you want to delete this recipe?")) {
        ...
        const response = await fetch(
          `http://localhost:5100/auth/recipe/${recipeId}`,
          {
            method: "DELETE",
          }
        );
  ...
  const handleAddToFavorites = async (recipeId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5100/auth/likedRecipes/${recipeId}`,
        {
          method: "POST",
        }
      );
    ...
    try {
      if (e.target.value) {
        let Searchedrecipes = await fetch(
          `http://localhost:5100/auth/searchRecipes/${e.target.value}`,
          {
            method: "GET",
            headers: {
